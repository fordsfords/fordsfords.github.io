...
my $site = mkdef($query{"SITE"});

print "Content-Type: text/html\n\r\n\r";
$| = 1;  # auto-flush stdout
print <<__EOF__;
<html><head><title>chklinks</title></head><body bgcolor="#FFFFFF">
__EOF__

if (length($site) > 0) {
	print STDERR "site=$site\n";
	print "<h2>site=$site</h2>\n<pre>\n";
	print "<p>This may take a while...</p>\n";
...
print <<__EOF__;
<hr>
<form action="/cgi-bin/chklinks" method="GET">
Site: <input type="text" name="SITE" value="$site" size="80">
<br><input type="submit" name="SUBMIT" value="SUBMIT">
</form>
<hr>
<p>This script was hacked by sford from the original at
<a href="http://sourceforge.net/projects/chklinks/">http://sourceforge.net/projects/chklinks/</a></p>
<p>Source for this hacked version can be found at <a href="http://geeky-boy.com/chklinks">http://geeky-boy.com/chklinks</a>
<p>Since this script puts a heavy load on my server, please use it sparingly and do not advertise it widely.</p>
</body></html>
__EOF__
...
sub main() {
    ...
    print "Done.  " . (time - $^T) . " seconds elapsed.\n"
        if $VERBOSE > 0;
    return;
}
...
sub fetch_noredir($) {
    ...
    print "Requesting $urirep... " if $VERBOSE > 2;
    $ua = get_ua $uritext;
    ...
    # 3XX Redirect
        ...
        if (!defined $r->header("Location")) {
            print "$urirep " . $r->status_line . ": Redirect without location\n";
            return (undef, undef);
        }
...
sub proc_link_val($$) {
    ...
    if (!exists $ALLOWED_SCHEMES{$uri->scheme}) {
        $_ = rep_uri $URIS{$base};
        print rep_uri($uri) . " (" . rep_uri($URIS{$base}) . "): "
